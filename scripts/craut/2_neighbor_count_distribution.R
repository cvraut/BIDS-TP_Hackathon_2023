...
ggplot2::ggsave(
  filename = "BIDS-TP_Hackathon_2023/scripts/craut/neighbor_distribution_uninfected_by_well_take2.pdf",
  plot = plot,
  width = 5,
  height = 3)

# generate neighbor graph
neighbor_graph48 <- cell_features_TS2PL1 |>
  dplyr::filter(time_point == "48 hours") |>
  compute_neighbor_graph_by_well(verbose = TRUE)

neighbor_distribution48 <- neighbor_graph48 |>
  dplyr::count(
    plate_id, row, column,
    cell1_well_index,
    name = "n_neighbors") |>
...
plot48 <- ggplot2::ggplot(data = neighbor_distribution48) +
  ggplot2::theme_bw() +
  ggplot2::geom_line(
    mapping = ggplot2::aes(
      x = n_neighbors,
...
ggplot2::ggsave(
  filename = "BIDS-TP_Hackathon_2023/scripts/craut/neighbor_distribution_uninfected_by_well_48.pdf",
  plot = plot,
  width = 5,
  height = 3)